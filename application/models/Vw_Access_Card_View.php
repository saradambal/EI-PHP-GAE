...
<!--            });-->
    ...
    function CV_load_customerdetails(custdetails_result)
    {
        ...
        {
            var CV_table_value='<table id="CV_tble_htmltable" border="1"  cellspacing="0" data-class="table" class="srcresult"><thead bgcolor="#6495ed" style="color:white"><tr><th>UNIT NO</th><th>ACTIVE CARDS</th><th>OLD CARDS</th><th>ACCESS REASON</th><th>ACCESS COMMENTS</th></tr></thead><tbody>';
            for(var i=0;i<CV_custcard_array.length;i++){
                ...
                CV_table_value+='<tr><td>'+CV_values.unitno+'</td><td>'+CV_active+'</td><td>'+CV_lost+'</td><td>'+CV_values.reason+'</td><td>'+CV_values.comments+'</td></tr>';
            }
    ...
    function CV_loadcardno_details(card_details)
    {
        ...
        if($('#CV_lb_searchby').val()==18)
        {
            if(CV_cardno!="SELECT")
            {
                if(CV_result_array.length!=0){
                    ...
                    var CV_table_header='<table id="CV_tble_htmltable" border="1" cellspacing="0" data-class="table" class="srcresult"><thead bgcolor="#6495ed" style="color:white"><tr><th style="width:150px">UNIT NO</th><th style="width:150px">ACTIVE CARDS</th><th style="width:150px">NON ACTIVE CARDS</th><th style="width:150px">OLD CARDS</th><th style="width:150px">ACCESS REASON</th><th style="width:300px">COMMENTS</th></tr></thead><tbody>';
                    CV_table_header+='<tr><td>'+CV_result_array[0].unitno+'<td>'+CV_active+'</td><td>'+CV_inventory+'</td><td>'+CV_lost+'</td><td>'+CV_result_array[0].reason+'</td><td>'+CV_result_array[0].comments+'</td></tr></tbody></table>';
        ...
        else if($('#CV_lb_searchby').val()==31)
        {
            if(CV_unitno!="SELECT"){
                if(CV_result_array.length!=0){
                    var CV_table_header='<table id="CV_tble_htmltable" border="1" cellspacing="0" data-class="table" class="srcresult"><thead bgcolor="#6495ed" style="color:white"><tr><th>ACTIVE CARDS</th><th>NON ACTIVE CARDS</th><th>OLD CARDS</th><th>ACCESS REASON</th></tr></thead><tbody>';
                    var CV_active=[];
                    var CV_inventory=[];
                    var CV_lost=[];
                    var CV_reason=[];
                    ...
                    for(var i=0;i<len;i++){
                        ...
                        CV_table_header+='<tr><td>'+active_card+'</td><td>'+inventory_card+'</td><td>'+lost_card+'</td><td>'+reason+'</td></tr>';
                    }
            ...
            if(CV_result_array.length!=0){
                var CV_table_header='<table id="CV_tble_htmltable" border="1" cellspacing="0" data-class="table" class="srcresult"><thead bgcolor="#6495ed" style="color:white"><tr><th>UNIT NO</th><th>ACTIVE CARDS</th><th>NON ACTIVE CARDS</th><th>CUSTOMER LOST CARDS</th><th>LOST CARDS</th><th>ACCESS REASON</th></tr><tbody>';
                for(var i=0;i<CV_result_array.length;i++)
                {
                    ...
                    CV_table_header+='<tr><td>'+CV_values.unitno+'</td><td>'+CV_active+'</td><td>'+CV_inventory+'</td><td>'+CV_customer_lost+'</td><td>'+CV_employee_lost+'</td><td>'+CV_values.reason+'</td></tr>';
                }
                CV_table_header+='</tbody></table>';
                $('section').html(CV_table_header);
                $('#CV_tble_htmltable').DataTable({
        ...
        <div class="panel-body">
            <fieldset>
                ...
                <div class="form-group" id="CV_custname" hidden>
                    ...
                    <div class="col-sm-6"><input type="text" name="CV_tb_custname" id="CV_tb_custname" class="auto CV_customerautovalidate form-control"/></div>
                ...
                <div class="form-group" id='CV_headerdiv' hidden>
                    <lable class="col-lg-10 srctitle" style="padding-top:15px;" id="CV_headermsg"></lable>
                </div>
