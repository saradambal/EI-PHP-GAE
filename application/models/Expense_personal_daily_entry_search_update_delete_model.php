<?php
Class Expense_personal_daily_entry_search_update_delete_model extends CI_Model {
...
//        $errormessage=$errmsg->result();
    ...
    public function PDLY_SEARCH_searchbybaby($PDLY_SEARCH_typelistvalue,$PDLY_SEARCH_startdate,$PDLY_SEARCH_enddate,$PDLY_SEARCH_babysearchoption,$PDLY_SEARCH_fromamount,$PDLY_SEARCH_toamount,$PDLY_SEARCH_searchcomments,$PDLY_SEARCH_invitemcom,$PDLY_SEARCH_invfromcomt,$PDLY_SEARCH_babycategory)
    {
//        $PDLY_SEARCH_selectquery=[];
    ...
    {
        $updatequery = "UPDATE EXPENSE_BABY SET EB_INVOICE_DATE='$babyinvdate',EB_AMOUNT='$babyinamt',EB_INVOICE_ITEMS='$babyinvitem',EB_INVOICE_FROM='$babyinfromt',EB_COMMENTS='$babycomment',ULD_ID=(SELECT ULD_ID FROM USER_LOGIN_DETAILS WHERE ULD_LOGINID='$USERSTAMP'),ECN_ID=(SELECT ECN_ID FROM EXPENSE_CONFIGURATION WHERE ECN_DATA='$babycategory') WHERE EB_ID='$ebid'";
    ...
    public  function expensecarupdate($ecid,$carcategory,$carinvdate,$carinamt,$carinfromt,$carinvitem,$carcomment,$USERSTAMP)
    {
        ...
        $updatequery = "UPDATE EXPENSE_CAR SET EC_INVOICE_DATE='$carinvdate',EC_AMOUNT='$carinamt',EC_INVOICE_ITEMS='$carinvitem',EC_INVOICE_FROM='$carinfromt',EC_COMMENTS='$carcomment',ULD_ID=(SELECT ULD_ID FROM USER_LOGIN_DETAILS WHERE ULD_LOGINID='$USERSTAMP'),ECN_ID=(SELECT ECN_ID FROM EXPENSE_CONFIGURATION WHERE ECN_DATA='$carcategory') WHERE EC_ID='$ecid'";
        $this->db->query($updatequery);
        if ($this->db->affected_rows() > 0) {
            return true;
        }
    ...
    {
        $updatequery = "UPDATE EXPENSE_PERSONAL SET EP_INVOICE_DATE='$personalinvdate',EP_AMOUNT='$personalinamt',EP_INVOICE_ITEMS='$personalinvitem',EP_INVOICE_FROM='$personalinfromt',EP_COMMENTS='$personalcomment',ULD_ID=(SELECT ULD_ID FROM USER_LOGIN_DETAILS WHERE ULD_LOGINID='$USERSTAMP'),ECN_ID=(SELECT ECN_ID FROM EXPENSE_CONFIGURATION WHERE ECN_DATA='$personalcategory') WHERE EP_ID='$epid'";
        $this->db->query($updatequery);
        if ($this->db->affected_rows() > 0) {
            return true;
        }
