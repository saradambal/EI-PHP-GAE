<?php
error_reporting(0);
class Mdl_erm extends CI_Model
{
    ...
    public function getAllDatas($Option,$Data1,$Data2)
    {
        if($Option==6)
        {
        $query=$this->db->query('SELECT ERM.ERM_ID,NC.NC_DATA,ERM.ERM_CUST_NAME,ERM.ERM_RENT,DATE_FORMAT(CONVERT_TZ(ERM.ERM_MOVING_DATE,"+00:00","+08:00"),"%d-%m-%Y") AS MOVING_DATE,ERM.ERM_MIN_STAY,EOD.ERMO_DATA,ERM.ERM_NO_OF_GUESTS,ERM.ERM_AGE,ERM.ERM_CONTACT_NO,ERM.ERM_EMAIL_ID,ERM.ERM_COMMENTS,ULD.ULD_LOGINID,DATE_FORMAT(CONVERT_TZ(ERM.ERM_TIMESTAMP,"+00:00","+08:00"),"%d-%m-%Y %T") AS ERM_TIME_STAMP FROM ERM_ENTRY_DETAILS ERM left join NATIONALITY_CONFIGURATION NC ON ERM.NC_ID=NC.NC_ID left join ERM_OCCUPATION_DETAILS EOD ON ERM.ERMO_ID=EOD.ERMO_ID left join USER_LOGIN_DETAILS ULD on ERM.ULD_ID=ULD.ULD_ID WHERE ULD.ULD_LOGINID="'.$Data1.'" ORDER BY ERM.ERM_MOVING_DATE,ERM.ERM_CUST_NAME');
        }
        elseif($Option==5)
        {
